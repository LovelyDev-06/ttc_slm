...
def main():
 p=argparse.ArgumentParser(); p.add_argument("--model",required=True,choices=["llama1b"]); p.add_argument("--dataset",default="arc_challenge",choices=["arc_challenge"]); p.add_argument("--split",default="train"); p.add_argument("--limit",type=int,default=None); p.add_argument("--seed",type=int,default=None,help="if set with --limit, take a reproducible random sample instead of the first N problems"); p.add_argument("--config",default="configs/config.yaml"); p.add_argument("--out",default="checkpoints/router.safetensors"); p.add_argument("--no_push",action="store_true"); p.add_argument("--fresh_net",action="store_true",help="retrain the network from scratch, reusing already-completed labels, without resuming from a prior net checkpoint"); a=p.parse_args()
