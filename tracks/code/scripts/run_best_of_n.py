#!/usr/bin/env python
"""Run Best-of-N with resumable JSON checkpoints and final CSV output."""
...
def restore_ledger(records):
    ledger = FlopLedger()
    for r in records:
        ledger.log(FlopRecord(r["strategy"], r["problem_id"], int(r["num_params"]), int(r["prompt_tokens"]), int(r["generated_tokens"]), int(r.get("num_generations",1)), int(r.get("extra_forward_passes",0)),int(r.get("extra_forward_tokens",0))))
    return ledger


def main():
    p=argparse.ArgumentParser()
    p.add_argument("--model",required=True,choices=["qwen1_5b","qwen7b"]); p.add_argument("--dataset",required=True,choices=["humaneval","mbpp"])
    ...
    results=state.get("results",[]); completed={r["problem_id"] for r in results}; ledger=restore_ledger(state.get("ledger_records",[]))
    ...
    for i,problem in enumerate(problems,1):
        ...
        if pid in completed: print(f"[{i}/{len(problems)}] {pid}: already complete, skipping"); continue
        print(f"[{i}/{len(problems)}] {pid}: running Best-of-N")
        ll=FlopLedger(); r=run_best_of_n(model,tokenizer,num_params,[problem],cfg,ll)[0]; fr=ll.as_dicts()[0]; r["flops"]=fr; results.append(r)
        ledger.log(FlopRecord(fr["strategy"],fr["problem_id"],int(fr["num_params"]),int(fr["prompt_tokens"]),int(fr["generated_tokens"]),int(fr["num_generations"]),int(fr["extra_forward_passes"]),int(fr.get("extra_forward_tokens",0))))
    ...
    write_results_csv(results,csv); print(f"Best-of-N pass rate: {sum(bool(r['passed']) for r in results)}/{len(results)} = {sum(bool(r['passed']) for r in results)/max(len(results),1):.2%}"); print(f"FLOP summary: {ledger.summary()}"); print(f"Wrote {csv}")
