...
def main():
    ...
    write_results_csv(results,csv); print(f"Tree-search pass rate: {sum(bool(r['passed']) for r in results)}/{len(results)} = {sum(bool(r['passed']) for r in results)/max(len(results),1):.2%}"); print(f"FLOP summary: {ledger.summary()}"); print(f"Wrote {csv}")
