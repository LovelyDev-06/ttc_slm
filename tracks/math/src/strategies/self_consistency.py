...
from src.math_utils import normalize_math, evaluate_output
...
def run_self_consistency(model,tokenizer,num_params,problems,config,ledger):
    ...
    for p in problems:
        ...
        for row in out:
            ids=row[inp["input_ids"].shape[1]:]; total+=len(ids); raw=tokenizer.decode(ids,skip_special_tokens=True); groups[normalize_math(raw,p.get("choices"))].append(raw)
