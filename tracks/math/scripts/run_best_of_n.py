...
def main():
    ...
    p.add_argument("--model",required=True,choices=["llama1b","llama3b"]); p.add_argument("--dataset",required=True,choices=["gsm8k","math"])
    ...
    write_results_csv(results,csv); print(f"Best-of-N accuracy: {sum(bool(r['passed']) for r in results)}/{len(results)} = {sum(bool(r['passed']) for r in results)/max(len(results),1):.2%}"); print(f"FLOP summary: {ledger.summary()}"); print(f"Wrote {csv}")
