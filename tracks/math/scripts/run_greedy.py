...
def main():
    ...
    parser.add_argument("--model", required=True, choices=["llama1b", "qwen1_5b"])
    ...
    print(f"Greedy accuracy: {sum(bool(r['passed']) for r in results)}/{len(results)} = {sum(bool(r['passed']) for r in results)/max(len(results),1):.2%}")
